...
En esta sección se priorizará el uso de la librería `terra` para la manipulación y análisis de datos espaciales, aunque también se utilizarán las librería `sf` y `raster` para algunas funciones específicas. En la manipulación de tablas y datos en general se trabajará con `dplyr`.
...
En primer lugar se importará el **archivo raster dNBR** que fue generado con en la sección anterior `rgee`. Se asume que luego de exportado a Google Drive, fue descargado a un directorio en la PC local. 
...
knitr::include_graphics("https://github.com/romina-gonzalez-musso/Severidad_Incendio-Steffen-Martin22/blob/master/_images/2_NBRvsAreaQuemada.png?raw=true")
```

Se observa que quedan píxeles aislados clasificados como área quemada. A continuación se realizarán una serie de pasos a fin de **filtrar y obtener el perímetro final.** 
...
Se seleccionan los polígonos de mayor tamaño que representarán la mayor parte del área del incendio. En este caso, son dos polígonos. 
...
Ahora se generará un **buffer** alrededor del estos dos polígonos principales, a fin de poder incluir como parte de la superficie quemada los polígonos más chicos. 
...
```{r, echo=FALSE, out.width="90%", fig.cap=""}
knitr::include_graphics("https://github.com/romina-gonzalez-musso/Severidad_Incendio-Steffen-Martin22/blob/master/_images/2_buffer.png?raw=true")
```


Se seleccionarán todos los polígonos incluidos en el área buffer que corresponden a píxeles quemados. 
...
Finalmente, se eliminarán los píxeles más pequeños que aún siguen generando ruido. 
...
```{r, echo=FALSE, out.width="90%", fig.cap=""}
knitr::include_graphics("https://github.com/romina-gonzalez-musso/Severidad_Incendio-Steffen-Martin22/blob/master/_images/2_Perimetro_poligono_incendio.png?raw=true")
```
...
```{r, echo = FALSE, message=FALSE, warning=FALSE, include=TRUE, paged.print=FALSE, results = TRUE}

print(paste0("La superficie estimada del incendio es de ", round(as.numeric(sum(perimeter_f1$area)/10000),3), " hectáreas."))

```

### **2c. Clasificar el raster dNBR por clases de severidad USGS**

Primero se recorta el área del incendio con `mask`y `crop` del paquete `terra`: 
...
```{r, echo=FALSE, out.width="90%", fig.cap=""}
knitr::include_graphics("https://github.com/romina-gonzalez-musso/Severidad_Incendio-Steffen-Martin22/blob/master/_images/2_dNBR_croped.png?raw=true")
```

Luego se establecen los rangos de clasificación del raster en función a las categorías USGS y la paleta de colores:
...
```{r, echo=FALSE, out.width="90%", fig.cap=""}
knitr::include_graphics("https://github.com/romina-gonzalez-musso/Severidad_Incendio-Steffen-Martin22/blob/master/_images/2_Severidad.png?raw=true")
```

Se puede construir una *Tabla de Atributos Raster* (RAT, por sus siglas en inglés) para mejor representación de los resultados.
...
```{r, echo=FALSE, out.width="90%", fig.cap=""}
knitr::include_graphics("https://github.com/romina-gonzalez-musso/Severidad_Incendio-Steffen-Martin22/blob/master/_images/2_Severidad_clases.png?raw=true")
```

Se puede **exportar el raster clasificado** en formato `.tiff` para trabajar en forma local. 
...
Los pasos para generar la tabla de superficies por clases de severidad son:

- Poligonizar la clasificación
- Eliminar (si fuese necesario) las categorías de *"No quemado"* que pudieran quedar dentro del área del incendio. 
...
Se puede chequear que el área del polígono de incendio obtenido en el paso *2b* es igual a la suma de las categorías estimadas en este paso. Puede haber pequeñas diferencias producto de la eliminación de píxeles "no quemados" y las conversiones de formatos (raster a vectorial)
...
print(paste0("Superficie estimada del incendio con el polígono vectorial es de ", round(as.numeric(sum(perimeter_f1$area)/10000),1), " ha."))
```

```{r, echo = FALSE, message=FALSE, warning=FALSE, include=TRUE, paged.print=FALSE, results = TRUE}

print(paste0("La superficie de estimada a partir de la sumatoria de clases de severidad es de ", round(sum(class_usgs_f1$area_ha), 1), "ha"))

```

### **2e. Tabla de superficies por clases de severidad USGS**    

Se construye una tabla que contiene la superficie total por clase de severidad en unidades absolutas (hectáras) y en porcentaje. 
